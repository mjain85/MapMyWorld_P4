...
void process_image_callback(const sensor_msgs::Image img)
{
  ...
  if(totalWhitePixel>=3 && 10*totalWhitePixel < 8*img.height * img.step){
    ballCenter = (ballCenter/totalWhitePixel);
    int wd = (ballCenter % img.width);
    int ht = ballIndx / img.step;
    ROS_INFO("Ball Image Pixel1 %d, Width: %d, Height: %d, image:%d x %d" , (int)totalWhitePixel, wd, ht, img.width, img.height);
    if(3*wd < img.width)
      ang_z = 0.15;
    else if(3*wd > 2*img.width)
      ang_z = -0.15;
    else
	ang_z = 0.1*(wd*2.0/img.width-1);

    lin_x = 0.1;
    lin_x = lin_x*(1.0 - 1.0*totalWhitePixel/(img.height * img.step));
    ang_z = ang_z*(1.0 - 1.0*totalWhitePixel/(img.height * img.step));
